...
function getWeekNumberByDate(date) {
  ...
  const diffToThursday = (dayOfWeek + 6) % 7;
  tempDate.setUTCDate(tempDate.getUTCDate() - diffToThursday + 3);

  const firstThursday = new Date(Date.UTC(tempDate.getUTCFullYear(), 0, 4));
  const firstThursdayDay = firstThursday.getUTCDay();
  const firstThursdayOffset = (firstThursdayDay + 6) % 7;
  firstThursday.setUTCDate(
    firstThursday.getUTCDate() - firstThursdayOffset + 3
  );
...
function getNextFridayThe13th(date) {
  ...
  while (candidate <= start || candidate.getDay() !== 5) {
    month += 1;
    if (month > 11) {
      month = 0;
      year += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (current <= endDate) {
    for (let i = 0; i < countWorkDays && current <= endDate; i += 1) {
      schedule.push(formatDat(current));
